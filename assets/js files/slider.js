...
let createSlider=document.getElementById("createSlider")
                ...
                document.querySelectorAll(".update").forEach(btn=>
                    {
                        btn.addEventListener("click", async function(e){
                            ...
                            if(catResponse.ok){
                                const category=await catResponse.json();
                                Array.from(updateSlider.elements).forEach(
                                    element=>{
                                        if(element.name){
                                            element.value=category[element.name]
                                        }
...
createSlider.addEventListener("submit", async function(e) {
    e.preventDefault();
...
updateSlider.addEventListener("submit", async function(e){
    e.preventDefault();
    const formData = new FormData(e.target);
    const cat = Object.fromEntries(formData.entries());
